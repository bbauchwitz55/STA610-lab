...
First, we examine the `ppm` variable (price per milligram), our primary outcome variable. Even though the values don’t vary over many orders of magnitude, it has extreme right skew. Taking the log of these values appears to dramatically improve various indicators of normality. While the data still fails a Shapiro-Wilk normality assessment this transform appears reasonable for the data, and the skew are kurtosis are both substantially resolved. 

**2. Location Grouping Variables** 

After processing the ‘city’ data there are 1654 unique cities represented. To explore whether there is a relationship between the cities and the log price per milligram of morphine, we make a box plot of randomly chosen 25 cities. The log(price) of morphine seems to differ by cities. Additionally, there is significant heterogeneity in the sample sizes across cities. 954 cities have only a single observation and the vast majority have fewer than 10, but the city with the most observations has over 300. Next, we explore the relationship between log(price) and state. Again, we observe heterogeneity of the state level price means, though variation in the sample sizes is much less severe. Finally, we explore the relationship between log(price) and region. There is some variation across regions, though the magnitude is fairly small (< 10 cents per mg in the untransformed data).

However, to determine which one of these grouping variables should be included in the model, we need to perform formal tests and analyses. Preliminarily, we make three models with cities, states, and regions as the only predictors respectively, and perform ANOVA tests. The results show that log ppm differs significantly by states and regions, but not that much by cities. We then build three random effects anova models with only cities, states, and regions as predictors respectively. The plots of random effects show that state might be the best to included individually as our grouping variables, it indicates reasonable amount of information to demonstrates the heterogeneity of log ppm across locations.**(See Appendix for random effect plots for cities and regions)** A series of nested tests for including multiply grouping variables will be performed in the Model section.   

```{r, echo = F, fig.asp=0.55, fig.cap="log ppm differs by cities"}
## city_tallies <- data %>% group_by(City_final) %>% tally()
## hist(city_tallies$n, breaks=100)
set.seed(100)
...
The first variable we consider is the linear temporal variable we created, which is number of days since Jan 1, 2010. This lets us encode time in a semi-continuous manner. Despite the discrete encoding, the data actually functions quite well as a numeric predictor since there is high resolution relative to the overall time scale. The only apparent thing is that when we plot the price data against over time, the data is clearly sparser earlier in streetrx's history. However, across the entire dataset we do not observe any clear relationship between time and log(price).**(See Appendix Fig.7)** We then consider the effects of days in each group, using state as our initial grouping predictor, by randomly sampling 8 states. In some states, the log ppm does seem to change over time, while in others it does not. Such change may not be very obvious as the data points are sparse. Still, this suggests it might be worth evaluating random slopes for time by state in our model.   
...
It seems that, in general, log(price) does not differ too much across different primary reasons for purchasing morphine. Within each state, the log ppm differs according to different primary purchase reasons. However, variance is significant, so variation may be largely a result of over segmentation of the data.  

##### Source (See Appendix Fig.10 and Fig.11)

It seems that in general, log(price) varies according to the values of the sources class, which indicates the source of each transactions recorded in the data set. By examining the associations between source class and log ppm in a random sample of states, we can see that such associations vary across states. Again, the within-source variance is quite large, so this effect may likely be the result of over segmentation, though it might be worth considering random slopes of sources class by states in the model.

##### Bulk (See Appendix Fig.12 and Fig.13)

The indicator for purchasing 10 units at once, Bulk, does not seem to have an obvious relationship with log ppm. We will not include a random interaction of bulk by state either as the plot does not show that such association vary by states. 
...
The overall conclusion is that price per milligram of morphine is primarily predicted by the dosage strength, the state in which the purchase was made, and whether or not it was a bulk purchase. Though the model cannot prove causation, this conclusion is plausible for several reasons. First, the active ingredient of a drug is usually the primary driver of price, so having price be related to dosage strength in a secondary market makes sense. Second, for most goods, bulk purchases reduce cost because it alleviates risk to the supplier, so it is not surprising to see that bulk purchases of morphine also tend to be at lower prices per milligram. Third, different states have different economic, regulatory, and social factors that will influence drug use, so it is not surprising to see that there are slightly different drug markets in different states.

We also interpret why certain factors may be absent from the model. The absence of ‘purchase reason’ and ‘source’ from the model are not that surprising. Purchase reason would not usually be known to the drug seller, so it makes sense that it should not significantly influence price. Similarly, source of information occurs after the transaction is complete and cannot influence the price, only the reporter’s recollection of the price. We do not have a strong hypothesis that prices should be reported more or less accurately from different sources of information. The absence of ‘city’ is likely due to the sample size of the data. Heterogeneity across locations is likely to increase at higher levels of resolution, so with a sufficiently large dataset we may see the largest location heterogeneity at the city level. However, when the vast majority of cities have only one or a few observations, it is not possible to detect these differences in the data. At lower levels of resolution, variation is likely to reduce, so it also makes sense that region is not a significant predictor. The absence of ‘month’ as a predictor suggests that seasonal trends do not strongly predict price. For a drug with major dependence on agricultural precursors, such as heroin or cocaine, we may expect more monthly trends, but for a drug like morphine it is less surprising that monthly trends are weak. The absence of time affecting the price is most surprising. For most goods, we expect that prices will increase over time due to inflation. However, for high-tech goods, we also observe prices decreasing over time as production efficiency increases. The fact that time does not strongly predict price may be explained by these two opposing forces canceling each other out.
